...
presentation/
  ...
  widgets/    # HomeTopBar, HomeTopBarHeaderDelegate, HomeBottomNavBar,
              # HomePosterCard, HomeCollectionCard, HomeLoopingCarousel, and
              # one *_section.dart per content section
```
...
- The bottom nav's Profile tab calls `ref.read(authViewModelProvider.notifier).signOut()` directly (from `features/auth/`) — this is the one place `home/` reaches into `auth/`'s presentation providers. That call now makes a network round-trip (`POST /auth/logout` to revoke the refresh token — see `lib/features/auth/CLAUDE.md`); the tap site doesn't await it or show a loading/disabled state, which is harmless today (the revoke is best-effort and `AuthGate` swaps the screen away on completion) but is where a spinner would go if one's ever wanted.
...
- `HomeFeaturedCollectionsSection` and `HomeEndingSoonSection` scroll horizontally in an infinite loop (`HomeLoopingCarousel<T>`) rather than a bounded `ListView`. No carousel package is a dependency — it's hand-rolled: `ListView.builder(itemExtent: itemWidth + spacing, itemCount: items.length * 100000)` with `items[index % items.length]` resolving each built index, and a `ScrollController(initialScrollOffset: ...)` computed to land exactly on a repeat boundary so it looks identical to a plain list at rest. **Always keep `itemExtent` set** — without it, `ListView` can't jump directly to a large `initialScrollOffset` and instead estimates/lays out from index 0, which is catastrophically slow (confirmed: removing it hangs the app). Because 3 real items repeat within a typical viewport, tests that assert on carousel item text must use `findsAtLeastNWidgets(1)`, not `findsOneWidget` — duplication at rest is correct looping behavior, not a bug.
